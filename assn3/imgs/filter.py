...
def mean(X):
  ...
  filter_sz = 3
  R = np.ones(filter_sz**2)
  normalization = np.sum(R)
  R = (1/normalization)*R.reshape(filter_sz, filter_sz)
...
def gaussianHP(X):
  ...
  std=2
  return X - ndimage.gaussian_filter(X,std)
  # END YOUR CODE HERE
